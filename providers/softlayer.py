...
# And why is this convoluted shit EASIER than IBM Bluemix API
...
def NextBilling(account_name, api_key) -> CostItem:
    ...
    # For documentation purposes another way to do this:
    # Filter for only categories guest_storage and guest_core
    # Didn't use this in the event Softlayer decides to charge
    # more categories than guest_storage, guest_core
    # Can grab a list of all
    onlyGuests = {
        'nextInvoiceTopLevelBillingItems': {
            'categoryCode' : {
                'operation': 'in',
                'options': [{
                    'name': 'data',
                    'value': ['guest_storage','guest_core']
                }]
            }
        }
    }
